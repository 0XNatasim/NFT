...
if (VERIFIER === "sourcify") {
  // Some Foundry builds still auto-select the Etherscan verifier when an
  // ETHERSCAN_API_KEY is present in the environment, even if --verifier
  // sourcify is passed. Hide Etherscan-only env from the child process so the
  // CLI cannot silently override the requested Monad Sourcify verifier.
  delete forgeEnv.ETHERSCAN_API_KEY;
  delete forgeEnv.ETHERSCAN_KEY;
}

console.log(`\nRunning: forge ${forgeArgs.join(" ")}\n`);
run("forge", forgeArgs, { stdio: "inherit", env: forgeEnv });
