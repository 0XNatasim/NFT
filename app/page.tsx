        ...
        {carouselSlides.map((slide, index) => {
          ...
          return (
            <Card
              ...
              className="hero-carousel-card absolute left-1/2 top-1/2 h-[25rem] w-[min(82vw,26rem)] overflow-hidden border-monad-purple/30 bg-gradient-to-br from-card/95 via-monad-purple/10 to-cyan-400/10 shadow-2xl shadow-monad-purple/10"
              style={{
...
function CustomDealSlide() {
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-3 rounded-2xl border border-cyan-300/20 bg-background/60 p-3">
      <div className="grid grid-cols-3 gap-2 text-[10px] uppercase tracking-wide text-foreground/95">
        <div className="rounded-lg border border-border bg-secondary/50 p-2">
          <span className="block text-foreground/90">Page 1</span>
          <span className="font-semibold normal-case text-foreground">Buy with MON</span>
        </div>
        <div className="rounded-lg border border-monad-purple bg-monad-purple/20 p-2 shadow-sm shadow-monad-purple/20">
          <span className="block text-monad-purple">Page 2</span>
          <span className="font-semibold normal-case text-foreground">Custom trade</span>
        </div>
        <div className="rounded-lg border border-border bg-secondary/50 p-2">
          <span className="block text-foreground/90">Page 3</span>
          <span className="font-semibold normal-case text-foreground">Wanted board</span>
        </div>
      </div>

      <div className="flex min-h-0 flex-1 flex-col rounded-xl border border-monad-purple/25 bg-card/80 p-3">
        <div className="mb-3 flex items-center gap-3">
          <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-monad-purple text-monad-black">
            <SlidersHorizontal className="h-5 w-5" />
          </div>
          <div>
            <p className="text-xs font-semibold uppercase tracking-wide text-monad-purple">
              Page 2 of 3
            </p>
            <h4 className="text-base font-semibold">Create a custom trade</h4>
          </div>
        </div>

        <div className="rounded-lg border border-monad-purple/30 bg-monad-purple/10 px-3 py-2 text-sm font-semibold text-monad-purple">
          Example: trade 1 10kSquad + 10,000 MON for 1 r3tard
        </div>

        <div className="mt-3 grid grid-cols-[1fr_auto_1fr] items-center gap-2">
          <TradeMiniPanel
            title="You give"
            items={["1 10kSquad NFT", "10,000 MON"]}
          />
          <div className="flex h-8 w-8 items-center justify-center rounded-full bg-monad-purple text-monad-black">
            <Handshake className="h-4 w-4" />
          </div>
          <TradeMiniPanel title="You get" items={["1 r3tard NFT"]} />
        </div>

        <ol className="mt-3 space-y-1 text-xs leading-5 text-foreground/95">
          <li>1. Choose Custom Trade in the builder.</li>
          <li>2. Add NFTs and MON on either side.</li>
          <li>3. Handshake settles only if both wallets still match.</li>
        </ol>
      </div>
    </div>
  );
    ...
    <div className="rounded-lg border border-cyan-300/20 bg-cyan-300/10 p-2">
      ...
      <div className="space-y-1">
        {items.map((item) => (
          <div
            key={item}
            className="rounded-md border border-monad-purple/20 bg-background/60 px-2 py-1 text-xs text-foreground/95"
          >
            {item}
          </div>
        ))}
